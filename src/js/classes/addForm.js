export default class AddForm {
    ...
    this.form.addEventListener("submit", async (ev) => {
      ...
      try {
        const response = await fetch(
          "http://localhost:7077?method=createTicket",
          {
            method: "POST",
            body: body,
          }
  ...
  `
    ...
    maket
      ...
      .addEventListener("click", async (ev) => {
        ...
        const card = await this.getCard(
          maket.querySelector(".card-checkbox").id
        );
    ...
    maket.addEventListener("click", async (ev) => {
      if (
        !ev.target.classList.contains("card-checkbox") &&
        !ev.target.classList.contains("action") &&
